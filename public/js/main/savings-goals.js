document.addEventListener('DOMContentLoaded', function () {
    // Pastikan window.pageData sudah didefinisikan dari Blade
    ...
    console.log('Savings Goals DOMContentLoaded. Page Data:', pageData); // DEBUG
    ...
    if (editSavingsGoalModalEl) {
        console.log('Event listener untuk editSavingsGoalModal ditambahkan.'); // DEBUG
        editSavingsGoalModalEl.addEventListener('show.bs.modal', function (event) {
            console.log('Modal Edit Tujuan dibuka.'); // DEBUG
            ...
            console.log('Edit Goal ID:', goalId); // DEBUG

            form.action = `${pageData.baseUrl}/${goalId}`;
            console.log('Edit Form Action di-set ke:', form.action); // DEBUG
        ...
        console.error("Elemen modal dengan ID 'editSavingsGoalModal' tidak ditemukan."); // DEBUG
    }
    ...
    if (addFundsModalEl) {
        console.log('Event listener untuk addFundsModal ditambahkan.'); // DEBUG
        addFundsModalEl.addEventListener('show.bs.modal', function(event) {
            console.log('Modal Tambah Dana dibuka.'); // DEBUG
            const button = event.relatedTarget; // Tombol "+ Dana" yang diklik
            if (!button) {
            ...
            console.log('Tambah Dana untuk Goal ID:', goalId, 'Nama:', goalName); // DEBUG

            document.getElementById('addFundsGoalName').textContent = goalName;
            form.action = `${pageData.baseUrl}/${goalId}/add-funds`;
            console.log('Add Funds Form Action di-set ke:', form.action); // DEBUG
        ...
        console.error("Elemen modal dengan ID 'addFundsModal' tidak ditemukan."); // DEBUG
    }

    // Otomatis membuka modal jika ada error validasi dari server
    if (pageData.session.openModalOnError) {
        console.log("Session 'openModalOnError' ada:", pageData.session.openModalOnError); // DEBUG
        let modalIdToOpen = pageData.session.openModalOnError;
        let errorGoalId = pageData.session.errorModalGoalId;
        console.log("Error Goal ID dari session:", errorGoalId); // DEBUG

        if (modalIdToOpen === 'addFundsModal' && errorGoalId) {
            console.log("Mencoba membuka modal Tambah Dana karena error validasi untuk Goal ID:", errorGoalId); // DEBUG
            ...
            if (pageData.errors.allBags && pageData.errors.allBags[errorBagName]) {
                ...
                console.log("Errors untuk modal "+errorBagName+":", errorsForModal); // DEBUG
            } else {
                console.warn("Error bag " + errorBagName + " tidak ditemukan di pageData.errors.allBags."); // DEBUG
            }
            ...
            if (modalElementToOpen) {
                ...
                else console.warn("Tidak bisa menemukan nama goal untuk error modal Tambah Dana, ID:", errorGoalId); // DEBUG
            } else {
                console.error("Elemen modal 'addFundsModal' tidak ditemukan saat mencoba membuka karena error."); // DEBUG
            }

        } else if (modalIdToOpen && modalIdToOpen.startsWith('editSavingsGoalModal-')) {
            console.log("Mencoba membuka modal Edit Tujuan karena error validasi:", modalIdToOpen); // DEBUG
             ...
             if (modalElementToOpen) {
                ...
                if(pageData.errors.hasUpdateErrors) { // Cek apakah ada error untuk update
                    console.log("Mengisi ulang form Edit Tujuan dengan old input dari pageData."); // DEBUG
                 ...
                 console.error("Elemen modal Edit Tujuan tidak ditemukan:", modalIdToOpen); // DEBUG
             }
        } else if (modalIdToOpen === 'addSavingsGoalModal' && pageData.errors.hasStoreErrors) {
            console.log("Mencoba membuka modal Tambah Tujuan Baru karena error validasi."); // DEBUG
            ...
            if (addGoalModalElement) {
                ...
                if (!modalInstance._isShown) { // Hanya buka jika belum terbuka
                    modalInstance.show();
                }
            } else {
